...
void callback_light(CoapPacket &packet, IPAddress ip, int port) {
  ...
  if (LEDSTATE) {
    digitalWrite(2, HIGH) ; 
    //coap.sendResponse(ip, port, packet.messageid, "ola");
    coap.sendResponse(ip, port, packet.messageid, "ola", 3, COAP_VALID, COAP_TEXT_PLAIN, packet.token, packet.tokenlen);
  } else { 
    digitalWrite(2, LOW) ; 
